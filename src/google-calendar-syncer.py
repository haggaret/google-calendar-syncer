...
def insert_into_calendar(service_client, from_cal, event, calendar, date_time_now, dryrun=False):
    ...
    new_event = {'id': new_event_id, 'start': event['start'], 'end': event['end']}
    ...
    if not dryrun:
        try:
            ...
            if 'The requested identifier already exists' in str(e):
                ...
                result = update_event_in_calendar(service_client, from_cal, event, calendar, date_time_now, dryrun)
    else:
...
def update_event_in_calendar(service_client, from_cal, event, calendar, date_time_now, dryrun=False):
    ...
    if f'Synced from {from_cal} by google-calendar-syncer.' not in updated_event_body['description']:
        updated_event_body['description'] += f'\n\nSynced from {from_cal} by google-calendar-syncer.'
    updated_event_body['description'] += f'\nLast updated on {date_time_now}'
    if 'location' in event:
...
def sync_events_to_calendar(service_client, last_sync, from_cal, from_cal_id, from_cal_cache, from_cal_events, to_cal,
    ...
    if from_cal_cache:
        ...
        logging.debug('Cached Calendar events: {}'.format(json.dumps(from_cal_cache)))
        ...
        for cache_event in from_cal_cache:
            ...
            if _should_be_excluded(cache_event, exclusions):
                ...
                for from_event in from_cal_events:
                    if _get_gcal_event_id(from_event['id']) == _get_gcal_event_id(cache_event['id']):
                        found_in_from = True
                        break
    ...
    if len(events_to_delete) == 0 and len(events_to_insert) == 0 and len(events_to_update) == 0:
        ...
        if len(events_to_delete) > 0:
            ...
            for old_event in events_to_delete:
                ...
                if not deleted:
                    ...
                    failed_deletes += 1
                    # TODO: How can we handle this so we try to delete it again next time?
                    #       Make sure it stays in the cache

            logging.info(f'Successfully deleted {str(len(events_to_delete) - failed_deletes)} old events from '
                         f'calendar: {to_cal}')
            if failed_deletes > 0:
                logging.info(f'Failed to deleted {str(failed_deletes)} old events from calendar: {to_cal}')
        ...
        if len(events_to_insert) > 0:
            ...
            for new_event in events_to_insert:
                inserted = insert_into_calendar(service_client, from_cal, new_event, to_cal, date_time_now, dryrun)
                if not inserted:
        ...
        if len(events_to_update) > 0:
            ...
            for update_event in events_to_update:
                updated = update_event_in_calendar(service_client, from_cal, update_event, to_cal, date_time_now,
                                                   dryrun)
                if not updated:
...
def sync_events(service_client, time, config, cache=None, refresh_cache=False, dryrun=False):
    ...
    for item in config:
        ...
        for src_cal in source_cals:
            ...
            logging.debug('      Source Calendar events: {}'.format(json.dumps(src_cal_events)))
            if refresh_cache:
                src_cal_cache = None
            else:
            ...
            sync_events_to_calendar(service_client,
                                    ...
                                    exclusions,
                                    dryrun)
            old_cache[src_cal_id] = (cache[src_cal_id] if cache and src_cal_id in cache else None)
